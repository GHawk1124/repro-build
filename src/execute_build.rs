...
pub async fn execute_nix_build(
    ...
    for target in targets {
        ...
        // Extract the package name - if it contains "packages." use the right format
        let package_ref = if target.starts_with("packages.") {
            target.to_string()
        } else {
            format!("default") // Use default package if not specified
        };
        ...
        if let Err(e) = execute_command(docker, container_id, &build_cmd).await {
            ...
            // Try with simplified package reference
