...
/// // In build.rs
/// fn main() {
///     if let Err(e) = repro_build_lib::build_script::run_build() {
...
pub async fn run_build() -> Result<()> {
    ...
    match result {
        Ok(_) => {
            let target_path = Path::new(&cargo_manifest_dir).join("target/repro-build");
            println!("cargo:warning=Build completed successfully!");
            
            if target_path.exists() {
                println!("cargo:warning=Build artifacts available in target/repro-build/");
                
                // Copy artifacts to OUT_DIR if requested
                if env::var("REPRO_BUILD_COPY_TO_OUT_DIR").unwrap_or_else(|_| "false".to_string()) == "true" {
